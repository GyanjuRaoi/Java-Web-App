	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (password.equals(confirmpass)) {
            if (userName == null || userName.isEmpty()) {
            	...
            	String key = "U3CdwubLD5yQbUOG92ZnHw=="; // You may adjust key 
                try {
					String encryptedPassword = PasswordEncryptionWIthAes.encryptPassword(password, key);
					String encryptedconfirmpass = PasswordEncryptionWIthAes.encryptPassword(confirmpass, key);
	            	UserModel user = new UserModel(userId, userName, firstName, lastName, email, encryptedPassword, encryptedconfirmpass);
                ...
                default:
					...
					e.printStackTrace();
				    request.setAttribute("errorMessage", "An error occurred while encrypting the passwords.");
				    request.getRequestDispatcher("/viewpage/crownregister.jsp").forward(request, response);
				}
            }
		}else {
